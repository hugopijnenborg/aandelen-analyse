...
// Elke metric heeft zones: slecht | zwak | neutraal | goed | sterk
// range: [min, max] van de schaal
// zones: [slecht_tot, zwak_tot, neutraal_tot, goed_tot] — daarna is het sterk
// lager_is_beter: voor metrics zoals P/E, D/E waar lager beter is
const METRICS = [
  {
...
// Bereken positie op de balk (0-100%)
...
// Bereken zone grenzen als percentages
...
styleEl.textContent = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .band-zone { flex: 1; border-radius: 2px; opacity: 0.35; transition: opacity 0.2s; }
...
function BandBar({ metric, value }) {
    ...
    return (
      <div className="band-container">
        <div className="band-track">
          {ZONE_COLORS.map((c, i) => (
            <div key={i} className="band-zone" style={{ background: c }} />
          ))}
        </div>
        <div className="band-labels">
          {metric.bandLabels.map((l, i) => (
            <span key={i} className="band-label">{l}</span>
          ))}
        </div>
      </div>
    )
  ...
  // Bereken zone grenzen als percentages van de balk
    ...
    <div className="band-container">
      <div className="band-track">
        {zoneWidths.map((w, i) => (
          <div
            key={i}
            className={`band-zone${i === activeZone ? ' active' : ''}`}
